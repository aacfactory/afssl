...
func processExtensions(out *Certificate) error {
	...
	for _, e := range out.Extensions {
		...
		if len(e.Id) == 4 && e.Id[0] == 2 && e.Id[1] == 5 && e.Id[2] == 29 {
			switch e.Id[3] {
			case 15:
				keyUsage, keyUsageErr := parseKeyUsageExtension(e.Value)
				if keyUsageErr != nil {
					return keyUsageErr
				}
				out.KeyUsage = x509.KeyUsage(keyUsage)
			case 19:
...
func parseCertificate(der []byte) (*Certificate, error) {
	...
	cert.SignatureAlgorithm = x509.SignatureAlgorithm(getSignatureAlgorithmFromAI(sigAI))
