...
const (
	...
	SM2WithSM3 x509.SignatureAlgorithm = 99
)

func (algo SignatureAlgorithm) isRSAPSS() bool {
	switch algo {
	case SignatureAlgorithm(SHA256WithRSAPSS), SignatureAlgorithm(SHA384WithRSAPSS), SignatureAlgorithm(SHA512WithRSAPSS):
		return true
	default:
		return false
	}
...
var signatureAlgorithmDetails = []struct {
	...
	{SignatureAlgorithm(MD2WithRSA), "MD2-RSA", oidSignatureMD2WithRSA, RSA, crypto.Hash(0) /* no value for MD2 */},
	{SignatureAlgorithm(MD5WithRSA), "MD5-RSA", oidSignatureMD5WithRSA, RSA, crypto.MD5},
	{SignatureAlgorithm(SHA1WithRSA), "SHA1-RSA", oidSignatureSHA1WithRSA, RSA, crypto.SHA1},
	{SignatureAlgorithm(SHA1WithRSA), "SHA1-RSA", oidISOSignatureSHA1WithRSA, RSA, crypto.SHA1},
	{SignatureAlgorithm(SHA256WithRSA), "SHA256-RSA", oidSignatureSHA256WithRSA, RSA, crypto.SHA256},
	{SignatureAlgorithm(SHA384WithRSA), "SHA384-RSA", oidSignatureSHA384WithRSA, RSA, crypto.SHA384},
	{SignatureAlgorithm(SHA512WithRSA), "SHA512-RSA", oidSignatureSHA512WithRSA, RSA, crypto.SHA512},
	{SignatureAlgorithm(SHA256WithRSAPSS), "SHA256-RSAPSS", oidSignatureRSAPSS, RSA, crypto.SHA256},
	{SignatureAlgorithm(SHA384WithRSAPSS), "SHA384-RSAPSS", oidSignatureRSAPSS, RSA, crypto.SHA384},
	{SignatureAlgorithm(SHA512WithRSAPSS), "SHA512-RSAPSS", oidSignatureRSAPSS, RSA, crypto.SHA512},
	{SignatureAlgorithm(DSAWithSHA1), "DSA-SHA1", oidSignatureDSAWithSHA1, DSA, crypto.SHA1},
	{SignatureAlgorithm(DSAWithSHA256), "DSA-SHA256", oidSignatureDSAWithSHA256, DSA, crypto.SHA256},
	{SignatureAlgorithm(ECDSAWithSHA1), "ECDSA-SHA1", oidSignatureECDSAWithSHA1, ECDSA, crypto.SHA1},
	{SignatureAlgorithm(ECDSAWithSHA256), "ECDSA-SHA256", oidSignatureECDSAWithSHA256, ECDSA, crypto.SHA256},
	{SignatureAlgorithm(ECDSAWithSHA384), "ECDSA-SHA384", oidSignatureECDSAWithSHA384, ECDSA, crypto.SHA384},
	{SignatureAlgorithm(ECDSAWithSHA512), "ECDSA-SHA512", oidSignatureECDSAWithSHA512, ECDSA, crypto.SHA512},
	{SignatureAlgorithm(PureEd25519), "Ed25519", oidSignatureEd25519, Ed25519, crypto.Hash(0) /* no pre-hashing */},

	{SignatureAlgorithm(SM2WithSM3), "SM2-SM3", oidSignatureSM2WithSM3, ECDSA, crypto.Hash(0) /* no pre-hashing */},
}
...
func (e InsecureAlgorithmError) Error() string {
	var override string
	if SignatureAlgorithm(e) == SignatureAlgorithm(SHA1WithRSA) || SignatureAlgorithm(e) == SignatureAlgorithm(ECDSAWithSHA1) {
...
func (c *Certificate) CheckSignatureFrom(parent *Certificate) error {
	...
	return checkSignature(SignatureAlgorithm(c.SignatureAlgorithm), c.RawTBSCertificate, c.Signature, parent.PublicKey, false)
}
...
func checkSignature(algo SignatureAlgorithm, signed, signature []byte, publicKey crypto.PublicKey, allowSHA1 bool) (err error) {
	...
	isSM2 := algo == SignatureAlgorithm(SM2WithSM3)
	for _, details := range signatureAlgorithmDetails {
		if details.algo == algo {
...
func buildCertExtensions(template *x509.Certificate, subjectIsEmpty bool, authorityKeyId []byte, subjectKeyId []byte) (ret []pkix.Extension, err error) {
	...
	if template.KeyUsage != 0 &&
		...
		ret[n], err = marshalKeyUsage(KeyUsage(template.KeyUsage))
		if err != nil {
			return nil, err
		}
		n++
...
func CreateCertificate(rand io.Reader, template, parent *Certificate, pub, priv any) ([]byte, error) {
	realTemplate, convertErr := toCertificate(template)
	if convertErr != nil {
		return nil, fmt.Errorf("x509: unsupported template parameter type: %T", template)
	}

	realParent, convertErr := toCertificate(parent)
	if convertErr != nil {
	...
	hashFunc, signatureAlgorithm, err := signingParamsForPublicKey(key.Public(), SignatureAlgorithm(realTemplate.SignatureAlgorithm))
	if err != nil {
		return nil, err
	}
	...
	if realTemplate.SignatureAlgorithm != 0 && SignatureAlgorithm(realTemplate.SignatureAlgorithm).isRSAPSS() {
		signerOpts = &rsa.PSSOptions{
			SaltLength: rsa.PSSSaltLengthEqualsHash,
			Hash:       hashFunc,
		}
	} else if signatureAlgorithm.Algorithm.Equal(oidSignatureSM2WithSM3) {
		signerOpts = sm2.DefaultSignerOpts
	}
...
func parseCertificateRequest(in *certificateRequest) (*CertificateRequest, error) {
	out := &CertificateRequest{
		...
		SignatureAlgorithm: x509.SignatureAlgorithm(getSignatureAlgorithmFromAI(in.SignatureAlgorithm)),
...
func (c *CertificateRequest) CheckSignature() error {
	return checkSignature(SignatureAlgorithm(c.SignatureAlgorithm), c.RawTBSCertificateRequest, c.Signature, c.PublicKey, true)
}
...
func CreateRevocationList(rand io.Reader, template *x509.RevocationList, issuer *Certificate, priv crypto.Signer) ([]byte, error) {
	...
	hashFunc, signatureAlgorithm, err := signingParamsForPublicKey(priv.Public(), SignatureAlgorithm(template.SignatureAlgorithm))
	if err != nil {
		return nil, err
	}
	...
	var signerOpts crypto.SignerOpts = hashFunc
	if SignatureAlgorithm(template.SignatureAlgorithm).isRSAPSS() {
		signerOpts = &rsa.PSSOptions{
			SaltLength: rsa.PSSSaltLengthEqualsHash,
			Hash:       hashFunc,
		}
