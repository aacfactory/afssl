...
func checkChainTrustStatus(c *Certificate, chainCtx *syscall.CertChainContext) error {
	if chainCtx.TrustStatus.ErrorStatus != syscall.CERT_TRUST_NO_ERROR {
		...
		case syscall.CERT_TRUST_IS_NOT_TIME_VALID:
			return CertificateInvalidError{c.ToX509(), Expired, ""}
		case syscall.CERT_TRUST_IS_NOT_VALID_FOR_USAGE:
			return CertificateInvalidError{c.ToX509(), IncompatibleUsage, ""}
...
func checkChainSSLServerPolicy(c *Certificate, chainCtx *syscall.CertChainContext, opts *VerifyOptions) error {
	...
	if status.Error != 0 {
		...
		case syscall.CERT_E_CN_NO_MATCH:
			return HostnameError{c.ToX509(), opts.DNSName}
...
func verifyChain(c *Certificate, chainCtx *syscall.CertChainContext, opts *VerifyOptions) (chain []*Certificate, err error) {
	...
	for i, parent := range chain[1:] {
		...
		if err := parent.CheckSignature(SignatureAlgorithm(chain[i].SignatureAlgorithm),
